    ...
    public void AssertConfigurationIsValid(TypeMap[] typeMaps)
    {
        ...
        var badTypeMaps =
            (from typeMap in typeMaps
             ...
             select new MapZillaConfigurationException.TypeMapConfigErrors(typeMap, unmappedPropertyNames, canConstruct)).ToArray();
        if (badTypeMaps.Length > 0)
        {
            configExceptions.Add(new MapZillaConfigurationException(badTypeMaps));
        }
        ...
        void DryRunTypeMap(TypePair types, TypeMap typeMap, MemberMap memberMap)
        {
            ...
            else
            {
                ...
                if (mapperToUse == null)
                {
                    configExceptions.Add(new MapZillaConfigurationException(memberMap.TypeMap.Types) { MemberMap = memberMap });
                    return;
                }
