...
{
    /// <summary>
    /// Dry run all configured type maps and throw <see cref="MapZillaConfigurationException"/> for each problem
    /// </summary>
    ...
    /// See <a href="https://MapZilla.readthedocs.io/en/latest/Understanding-your-mapping.html">the wiki</a> for details.
    /// </summary>
    ...
    /// Use if you want MapZilla to compile all mappings up front instead of deferring expression compilation for each first map.
    /// </summary>
...
public sealed class MapperConfiguration : IGlobalConfiguration
{
    ...
    LambdaExpression IGlobalConfiguration.BuildExecutionPlan(in MapRequest mapRequest)
    {
        ...
        LambdaExpression GenerateObjectMapperExpression(in MapRequest mapRequest, IObjectMapper mapper)
        {
            ...
            if (mapper == null)
            {
                MapZillaMappingException exception = new("Missing type map configuration or unsupported mapping.", null, mapRequest.RuntimeTypes)
                {
    ...
    public static MapZillaMappingException GetMappingError(Exception innerException, in MapRequest mapRequest) =>
