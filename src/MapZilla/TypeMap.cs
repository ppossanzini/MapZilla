...
public sealed class TypeMap
{
    ...
    public string CheckRecord() => ConstructorMap?.Ctor is ConstructorInfo ctor && ctor.IsFamily && ctor.Has<CompilerGeneratedAttribute>() ?
        " When mapping to records, consider using only public constructors. See https://docs.MapZilla.org/en/latest/Construction.html." : null;
    ...
    public void CheckProjection()
    {
        if (Projection)
        {
            throw new MapZillaConfigurationException("CreateProjection works with ProjectTo, not with Map.", MissingMapException(Types));
        }
    }
    ...
    public IEnumerable<MemberMap> MemberMaps
    {
        get
        {
            var maps = Enumerable.Concat(PropertyMaps, (IReadOnlyCollection<MemberMap>)PathMaps);
    ...
    public Type MakeGenericType(Type type) => type.IsGenericTypeDefinition ?
        type.MakeGenericType(Enumerable.Concat(SourceType.GenericTypeArguments, DestinationType.GenericTypeArguments).Take(type.GenericParametersCount()).ToArray()) :
        type;
