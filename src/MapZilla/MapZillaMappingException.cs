...
/// </summary>
public class MapZillaMappingException : Exception
{
    private readonly string _message;

    public MapZillaMappingException()
    {
    }

    public MapZillaMappingException(string message)
        : base(message) => _message = message;

    public MapZillaMappingException(string message, Exception innerException)
        : base(message, innerException) => _message = message;

    public MapZillaMappingException(string message, Exception innerException, TypePair types)
        : this(message, innerException) => Types = types;

    public MapZillaMappingException(string message, Exception innerException, TypeMap typeMap)
        : this(message, innerException, typeMap.Types) => TypeMap = typeMap;

    public MapZillaMappingException(string message, Exception innerException, MemberMap memberMap)
        ...
        get
        {
            return string.Join(Environment.NewLine,
                base.StackTrace
                    ...
                    .Where(str => !str.TrimStart().StartsWith("at MapZilla.")));
        }
    }
#endif
}
...
public class MapZillaConfigurationException : Exception
{
    ...
    public MapZillaConfigurationException(string message)
        : base(message)
    {
    }

    public MapZillaConfigurationException(string message, Exception inner)
        : base(message, inner)
    {
    }

    public MapZillaConfigurationException(TypeMapConfigErrors[] errors) => Errors = errors;

    public MapZillaConfigurationException(TypePair types) => Types = types;
    ...
    public override string Message
    {
        get
        {
            if (Types.HasValue)
            {
                ...
                while (exToUse != null)
                {
                    if (exToUse is MapZillaConfigurationException configExc)
                    {
